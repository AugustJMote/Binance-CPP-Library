...
void OrderBook::MainThread(const std::string tradingPair) {
  ...
  try {
    ...
    while (_running && stream.read(buffer)) {
      ...
      buffer.clear();
      ParseUpdate(json);
      if (std::chrono::system_clock::now() - _streamStarted >
          std::chrono::hours(23)) {
        _endpoints = _resolver.resolve("stream.binance.us", "9443");
        auto ep = boost::asio::connect(get_lowest_layer(stream), _endpoints);
        stream.next_layer().handshake(boost::asio::ssl::stream_base::client);
        stream.handshake("stream.binance.us:9443",
                         "/ws/" + tradingPair + "@depth");
        _streamStarted = std::chrono::system_clock::now();
      }
    }
...
void OrderBook::ParseUpdate(const std::string &json) {
  try {
    ...
    for (size_t i = 0; i < json.size() - 2; ++i) {
      if (json.at(i) == '"' && json.at(i + 2) == '"') {
        if (json.at(i + 1) == 'b' || json.at(i + 1) == 'a') {
          char bidOrAsk = json.at(i + 1);
          i += 2;
          do {
            i += 4;
            double price = 0;
            while (isdigit(json.at(++i))) {
              price *= 10;
              price += json.at(i) - 48;
            }
            double pricedec = 0;
            ...
            while (isdigit(json.at(++i))) {
              quantity *= 10;
              quantity += json.at(i) - 48;
            }
            double quantitydec = 0;
            while (isdigit(json.at(++i))) {
              quantitydec *= 10;
              quantitydec += json.at(i) - 48;
            }
...
void OrderBook::PrefetchDepth(std::string symbol) {
  ...
  std::string request = "GET /api/v3/depth?symbol=" + symbol +
                        " HTTP/1.1\r\nHost: api.binance.us\r\nConnection: "
                        "close\r\n\r\n";
